...
def animate_traj(x, y, z, e1bx, e1by, e1bz, e2bx, e2by, e2bz, e3bx, e3by, e3bz):
    ...
    def apply_center_of_mass_rotation(X, Y, Z, rotation_matrix, x_translation, y_translation, z_translation, x_cm):
        ...
        for i in range(X.shape[0]):
            for j in range(X.shape[1]):
                ...
                rotated_coordinates = rotation_matrix @ coordinates
                X_rotated_translated[i, j] = rotated_coordinates[0] + x_cm + x_translation
    ...
    for i in range(len(x)):
        ...
        X_nose_rotated, Y_nose_rotated, Z_nose_rotated = apply_center_of_mass_rotation(X_nose, Y_nose, Z_nose, rotation_matrix, x_translation, y_translation, z_translation, x_cm)
        X_body_rotated, Y_body_rotated, Z_body_rotated = apply_center_of_mass_rotation(X_body, Y_body, Z_body, rotation_matrix, x_translation, y_translation, z_translation, x_cm)
