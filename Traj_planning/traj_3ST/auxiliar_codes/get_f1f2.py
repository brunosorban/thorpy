...
def get_f1f2(x_dot_dot, y_dot_dot, x_3dot, y_3dot, x_4dot, y_4dot, params):
    ...
    gamma_dot_dot = get_gamma_2dot(
        x_dot_dot, y_dot_dot, x_3dot, y_3dot, x_4dot, y_4dot, params
    )
...
def get_f1f2_dot(
    ...
    f1_dot = (
        ...
        * (
            x_3dot * e1bx
            - x_dot_dot * gamma_dot * e1by
            + y_3dot * e1by
            + (y_dot_dot + g) * gamma_dot * e1bx
        )
        + 2 * gamma_dot * gamma_dot_dot * J_z / l_tvc
    )
...
#################### same equations with numpy
def get_f1f2_np(x_dot_dot, y_dot_dot, x_3dot, y_3dot, x_4dot, y_4dot, params):
    g = params["g"]
    m = params["m"]
    J_z = params["J_z"]
    l_tvc = params["l_tvc"]

    e1bx = x_dot_dot / np.sqrt(x_dot_dot**2 + (y_dot_dot + g) ** 2)
    e1by = (y_dot_dot + g) / np.sqrt(x_dot_dot**2 + (y_dot_dot + g) ** 2)
