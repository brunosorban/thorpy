...
def get_pos(coefs, t):
    return coefs[0] * t**6 + coefs[1] * t**5 + coefs[2] * t**4 + coefs[3] * t**3 + coefs[4] * t**2 + coefs[5] * t + coefs[6]

def get_vel(coefs, t):
    return 6 * coefs[0] * t**5 + 5 * coefs[1] * t**4 + 4 * coefs[2] * t**3 + 3 * coefs[3] * t**2 + 2 * coefs[4] * t + coefs[5]

def get_acc(coefs, t):
    return 30 * coefs[0] * t**4 + 20 * coefs[1] * t**3 + 12 * coefs[2] * t**2 + 6 * coefs[3] * t + 2 * coefs[4]
...
def calculate_trajectory(time_points, states, constraints):
    if len(time_points) != len(states["x"]):
        raise ValueError("The number of time points must be equal to the number of states")
    ...
    for k in range(0, number_of_points-1):
        # minimize the jerk
        obj += ca.power(Px_coefs[0, k], 2) + ca.power(Py_coefs[0, k], 2) + ca.power(Pgamma_coefs[0, k], 2)
    ...
    for k in range(0, number_of_points-1):
        ...
        opti.subject_to(F_pos_gamma(Pgamma_coefs[:, k], time_points[k]) == states["gamma"][k])
        ...
        opti.subject_to(F_pos_gamma(Pgamma_coefs[:, k], time_points[k+1]) == states["gamma"][k+1])
                
        if k < number_of_points - 2:
            # velocity constraints (continuity)
            opti.subject_to(F_vel(Px_coefs[:, k], time_points[k+1]) == F_vel(Px_coefs[:, k+1], time_points[k+1]))
            opti.subject_to(F_vel(Py_coefs[:, k], time_points[k+1]) == F_vel(Py_coefs[:, k+1], time_points[k+1]))
            opti.subject_to(F_vel_gamma(Pgamma_coefs[:, k], time_points[k+1]) == F_vel_gamma(Pgamma_coefs[:, k+1], time_points[k+1]))
                
            # acceleration constraints (continuity)
            opti.subject_to(F_acc(Px_coefs[:, k], time_points[k+1]) == F_acc(Px_coefs[:, k+1], time_points[k+1]))
            opti.subject_to(F_acc(Py_coefs[:, k], time_points[k+1]) == F_acc(Py_coefs[:, k+1], time_points[k+1]))
        ...
        if states["ax"][k] != None:
            ...
            print("Acceleration constraint added at t={:.1f} of ax={:.1f}".format(time_points[k], states["ax"][k]))
        if states["ay"][k] != None:
            opti.subject_to(F_acc(Py_coefs[:, k], time_points[k]) == states["ay"][k])
            print("Acceleration constraint added at t={:.1f} of ay={:.1f}".format(time_points[k], states["ay"][k]))
    ...
    opti.subject_to(F_vel_gamma(Pgamma_coefs[:, 0], time_points[0]) == constraints["gamma_dot0"])
    ...
    for i in range(number_of_points-1):
        ...
        for j in range(100):
            ...
            cur_interpolated_pos_x = states["x"][i] + (dx / t_interval) * (cur_time - time_points[i])
            cur_interpolated_pos_y = states["y"][i] + (dy / t_interval) * (cur_time - time_points[i])
            cur_interpolated_pos_gamma = states["gamma"][i] + (dgamma / t_interval) * (cur_time - time_points[i])
            ...
            # opti.subject_to(F_pos(Px_coefs[:, i], cur_time) < cur_interpolated_pos_x + constraints["offset"])
            # opti.subject_to(F_pos(Py_coefs[:, i], cur_time) < cur_interpolated_pos_y + constraints["offset"])
            opti.subject_to(F_pos_gamma(Pgamma_coefs[:, i], cur_time) > cur_interpolated_pos_gamma - constraints["angle_offset"])
            opti.subject_to(F_pos_gamma(Pgamma_coefs[:, i], cur_time) < cur_interpolated_pos_gamma + constraints["angle_offset"])
...
def plot_trajectory(time_points, states, Px_coefs, Py_coefs, Pgamma_coefs):
    ...
    for k in range(0, len(time_points)-1):
        ...
        gamma[t >= time_points[k]] = get_pos_gamma(Pgamma_coefs[:, k], t[t >= time_points[k]])
        ...
        gamma_dot[t >= time_points[k]] = get_vel_gamma(Pgamma_coefs[:, k], t[t >= time_points[k]])
    ...
    axs[2,0].plot(time_points, np.rad2deg(states["gamma"]), "o", label="$\\gamma_{ref}$")
