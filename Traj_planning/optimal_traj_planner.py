...
def calculate_trajectory(time_points, states, constraints):
    ...
    for k in range(0, number_of_points-1):
        ...
        opti.subject_to(F_pos(Pgamma_coefs[:, k], time_points[k+1]) == states["gamma"][k+1])
        ...
        if k < number_of_points - 2:
            ...
            opti.subject_to(F_vel(Pgamma_coefs[:, k], time_points[k+1]) == F_vel(Pgamma_coefs[:, k+1], time_points[k+1]))
            ...
            opti.subject_to(F_acc(Pgamma_coefs[:, k], time_points[k+1]) == F_acc(Pgamma_coefs[:, k+1], time_points[k+1]))
    ...
    opti.subject_to(F_vel(Pgamma_coefs[:, 0], time_points[0]) == constraints["gamma_dot0"])
    ...
    for i in range(number_of_points-1):
        ...
        for j in range(100):
            ...
            opti.subject_to(F_pos(Px_coefs[:, i], cur_time) < cur_interpolated_pos_x + constraints["offset"])
            opti.subject_to(F_pos(Py_coefs[:, i], cur_time) < cur_interpolated_pos_y + constraints["offset"])
            opti.subject_to(F_pos(Pgamma_coefs[:, i], cur_time) > cur_interpolated_pos_gamma - constraints["angle_offset"])
            opti.subject_to(F_pos(Pgamma_coefs[:, i], cur_time) < cur_interpolated_pos_gamma + constraints["angle_offset"])
...
def plot_trajectory(time_points, states, Px_coefs, Py_coefs, Pgamma_coefs):
    ...
    for k in range(0, len(time_points)-1):
        ...
        gamma_dot[t >= time_points[k]] = get_vel(Pgamma_coefs[:, k], t[t >= time_points[k]])
